...
/**
 * @file checkUser.php
 * @brief Kullanıcı Kimlik Doğrulama İşleyicisi
 *
 * Login formundan gelen POST verilerini işler, kullanıcı doğrulaması yapar.
 * Başarılı girişte session oluşturur ve dashboard'a yönlendirir.
 *
 * @author n0connect
 * @version 1.0.0
 * @date 2025-10-23
 ...
 *   - filter_var() ile email format kontrolü
 *   - password_verify() ile güvenli şifre kontrolü
 *   - Timing attack prevention (usleep)
...
// Kullanıcıyı bul
$stmt = $db->prepare("SELECT id, full_name, email, password, role, balance FROM User WHERE email = ?");
$stmt->execute([$email]);
...
// (usleep parametresi mikro saniye). // DoS riski taşıyor.
if ($elapsed_ms < 100) {
...
$_SESSION['login_time'] = time(); // Unix timestamp olarak
$_SESSION['login_datetime'] = date('Y-m-d H:i:s'); // Okunabilir format
$_SESSION['last_activity'] = time(); // Son aktivite zamanı (timeout için)

// User dashboard'a yönlendir
header('Location: dashboard.php?status=true');
exit;
?>